...
var ngMoveSelectionHandler = function($scope, elm, evt, grid) {
    ...
    var offset = 0;
    if (rowIndex !== 0 && (charCode === 38 || charCode === 9 && evt.shiftKey && firstInRow)) { //arrow key up or shift enter or tab key and first item in row
        offset = -1;
    }
    else if (rowIndex !== items.length - 1 && (charCode === 40 || charCode === 9 && lastInRow)) {//arrow key down, enter, or tab key and last item in row?
        offset = 1;
    }
    else if(charCode === 13){
        $scope.gridEnter(items[rowIndex].entity);
    }
